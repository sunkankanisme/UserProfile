...
object ExampleRfClassification {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val datasDF: DataFrame = spark.read.format("libsvm").load("datas/mllib/sample_libsvm_data.txt")
        ...
        // 自动识别特征数据中属于类别特征的字段，进行索引转换，决策树中使用类别特征更加 好
        ...
        //println(rfModel.featureImportance's) // 每个特征的重要性
        // TODO: 4. 模型评估
        val predictionDF: DataFrame = rfModel.transform(testingDF)
        predictionDF.select("prediction", "label_index").show(50, truncate = false)
