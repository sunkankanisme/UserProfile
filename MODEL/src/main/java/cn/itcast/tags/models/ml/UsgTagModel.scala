 ...
 * - 增加训练验证优化
 */
class UsgTagModel extends AbstractModel("用户购物性别", ModelType.ML) {
    override def doTag(businessDF: DataFrame, tagDF: DataFrame): DataFrame = {
         ...
         *  + 使用数据集构建管道模型
         *  - 使用训练验证分割获取最佳模型
         *    val pipelineModel = trainBestModel(goodsDF)
         *    val predictionDF = pipelineModel.transform(goodsDF)
         *  + 使用交叉验证的方式获取最佳模型，并实现模型的保存和加载
         */
        val model = MLModelTools.loadModel(goodsDF, "usg", this.getClass).asInstanceOf[PipelineModel]
        val predictionDF = model.transform(goodsDF)
