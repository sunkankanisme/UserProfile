...
/*
 * USG 模型开发，这里不使用机器学习算法，而是按照 统计+规则 进行打标签
 */
class UsgModel extends AbstractModel("用户购物性别", ModelType.ML) {
    override def doTag(businessDF: DataFrame, tagDF: DataFrame): DataFrame = {
         ...
         */
        val genderDF = goodsDF.groupBy($"userId").agg(
            // 某用户购买商品的总数
            count($"userId").as("total"),
            // 统计购买男性商品的个数
            sum(when($"label".equalTo(0), 1).otherwise(0)).as("maleTotal"),
            // 统计购买女性商品的个数
            sum(when($"label".equalTo(1), 1).otherwise(0)).as("femaleTotal")
        )
